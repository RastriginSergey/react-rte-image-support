...
type
ChangeHandler = (state: EditorState) => any;
...
export default class EditorToolbar extends Component {
    ...
    _setLink(url: string) {
        ...
        let linked = RichUtils.toggleLink(editorState, selection, entityKey)
        let collapsed = selection.merge({
                        anchorOffset: selection.getEndOffset(),
                        focusOffset: selection.getEndOffset()
                      });
        this.props.onChange(
            EditorState.forceSelection(linked, collapsed)
        );
        setTimeout(() => {
            this._focusEditor();
            let {editorState} = this.props;
            ...
            this.props.onChange(
                EditorState.push(editorState, cs, 'insert-text')
            );
        }, 10);
    }
